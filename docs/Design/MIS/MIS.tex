...
\newpage
\section{MIS of Load Data Module} \label{ReadDataModule} \wss{Use labels for
  cross-referencing}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
load\_sig & sig\_filename:string  & $<x_1, \dots, x_N>$ $x_n : \mathbb{R}$ & bad\_path, empty\_file, file\_wrong\_format \\
\hline
\end{tabular}
\end{center}
...
\subsubsection{Environment Variables}
\texttt{sig\_file} the file type variable of the external signal file
\wss{This section is not necessary for all modules.  Its purpose is to capture
  when the module has external interaction with the environment, such as for a
  device driver, screen interface, keyboard, file, etc.}
...
\noindent calc\_freq\_res($f_1$, $f_2$, win\_size):
\begin{itemize}
\item output: out := $ (f_2 - f_1) * win\_size$
\end{itemize}

\noindent calc\_time\_res($n_1$, $n_2$, win\_size):
\begin{itemize}
...
\noindent comp\_stft(sig):
where sig is the input signal $< x_1, \dots, x_N>$ where $x_n:\mathbb{R}$, $N:\mathbb{N}$
...
\subsubsection{Local Functions}
\noindent window\_function(WIN\_SIZE):
\begin{itemize}
...
\noindent comp\_waveletT(sig):
where sig is the $<x_1,\dots,x_N>$ where $ x_n:\mathbb{R}$, $N:\mathbb{N}$
\begin{itemize}
\item output: out := $<X_{0,0},X_{0,1},\dots, X_{1,0},X_{1,1},\dots, X_{I,J}>$ s.t. 
\begin{equation}
X(a,b) = \frac{1}{\sqrt{a}} \Sigma_{n = 0}^{wav\_scale\_a} w_{a,b,n} x_n 
\end{equation}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
plot\_matrix & $<X_{0,0},X_{0,1},\dots, X_{1,0},X_{1,1},\dots, X_{X,Y}>$ where $ x_{x,y}:\mathbb{}$   & $b:\mathbb{B}$ & bad\_path \\
...
\hline
calc\_colour & $x:\mathbb{R}$ & $R,B,G:\mathbb{N}$ & converts the matrix value into corresponding R,G,B values for heat map\\
